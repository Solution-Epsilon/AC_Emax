...
def plot_function(ax, beta, beta_float,ticks):
    ...
    ax.set_ylabel(r'$|N_t|$',fontsize = 16)
    x_plot = list(df["new"]).index(0)
    print(x_plot)
    ax.axvline(x = x_plot+1, color = "dimgrey", ls = "--", lw = 1)
    ax2 = ax.twinx()
    ax2.set_ylabel(r'$\left|X_t\right|$', fontsize = 16)
...
axes[0].plot([.1,.2,.3,.4,.5],[58/200,107/200,138/200,170/200,179/200],color="lightskyblue") 
axes[0].spines[['right', 'top']].set_visible(False)
